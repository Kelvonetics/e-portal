							...
							<div class="profile-right" style="width:75%">
								...
								<div class="custom-tabs-line tabs-line-bottom left-aligned">
									<ul class="nav" role="tablist">
										...
										<?php
											$conn = db_connect();	
											$query = "SELECT * FROM notifications WHERE notif_id = '{$nid}' AND status <> 'Pending Approval'";
											...
											<div class="panel table-responsive">
												...
												<?php
													...
													if(isset($_REQUEST['new_salary_btn']))
													{
														...
														$result = view_staff_salary_details($id);
													...
													if(isset($_REQUEST['staff_leave_btn']))
													{
														...
														if($leave_msg == 'leaveApplied')
														{
															echo '<div class="alert alert-success" style="width:97.5%; margin:auto">
																<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
																   <strong>Leave </strong> Your Application For Leave Was Sent Pending Approval.
																 </div>';
														}
														else
														{
											...
											<div class="panel">
												...
												<div class="panel-body">
													...
													<div class="input-group">
														...
														<input class="form-control" placeholder="Basic Salary" type="text" name="basicsalary" id="basicsalary" value="<?php echo @$salary_details['basicsalary']; ?>" <?php  if(@$_SESSION['role'] == 'Staff'){ echo 'readOnly'; } ?> required>
													</div>
													<br>
													<div class="input-group">
														<span class="input-group-addon"><i class="fa fa-money"></i></span>
														<select class="form-control" name="tax" id="tax" <?php  if(@$_SESSION['role'] == 'Staff'){ echo 'disabled'; } ?> required>
															<?php 
																if(@$_SESSION['role'] == 'Admin')
																{
																	$conn = db_connect(); 	
																	$query = "SELECT * FROM staff_salary_details WHERE staff_id = '{$aid}'";
																	$result = $conn->query($query);
																	WHILE($row = $result->fetch_assoc())
																	{
																		echo "<option class='option' value=\"".$row['tax']."\">".$row['tax'].' Percent'."</option>";
																	}
													...
													<div class="input-group">
														<span class="input-group-addon"><i class="fa fa-money"></i></span>
														<input class="form-control" placeholder="Monthly Deductions" type="text" name="deductions" id="deductions" value="<?php echo @$salary_details['deductions']; ?>"  <?php  if(@$_SESSION['role'] == 'Staff'){ echo 'readOnly'; } ?> required>
													</div>
													<br>
													<div class="input-group">
														<span class="input-group-addon"><i class="fa fa-bank"></i></span>
														<input class="form-control" placeholder="Bank Name" type="text" name="bank" id="bank" value="<?php echo @$salary_details['bank']; ?>" <?php  if(@$_SESSION['role'] == 'Staff'){ echo 'readOnly'; } ?> required>
													</div>
													<br>
													<div class="input-group">
														<span class="input-group-addon"><i class="fa fa-text">No</i></span>
														<input class="form-control" placeholder="Account Number" type="text" name="accountnumber" id="accountnumber" value="<?php echo @$salary_details['accountnumber']; ?>" <?php  if(@$_SESSION['role'] == 'Staff'){ echo 'readOnly'; } ?> required>
													</div>
													...
													<p> <span class="pull-left" style="font-size:15px"> Account Type 	</span> 	
														...
														<label class="fancy-radio pull-left" style="margin:auto 1% auto 7%">
															<input name="accounttype" name="savings" id="savings Account" value="Savings Account" <?php if(@$salary_details['accounttype'] == 'Savings Account'){ echo 'checked readOnly'; } if(@$_SESSION['role'] == 'Staff'){ echo 'disabled'; }  ?> type="radio">
															<span><i></i> Savings Account</span>
														</label>
														<label class="fancy-radio pull-left" style="margin:auto 1% auto 2%">
															<input name="accounttype" name="current" id="Current Account" value="Current Account" <?php if(@$salary_details['accounttype'] == 'Current Account'){ echo 'checked readOnly'; } if(@$_SESSION['role'] == 'Staff'){ echo 'disabled'; } ?> type="radio" required>
												...
												if(@$result->num_rows > 0)
												{
													...
													{  ?>
														<tr>
															...
															{  echo '<span class="fa fa-check btn-success act-btn"></span> Approved';  }
															elseif($row['status'] == 'Declined') 
															{ echo '<span class="fa fa-remove btn-danger act-btn"></span> Declined'; } ?> </td>
	...
	<!-- MODAL FORM TO RETRIEVE THE STAFF ATTENDANCE NUMBER  -->
	...
	<!-- MODAL FORM TO RETRIEVE THE STAFF ATTENDANCE NUMBER  --> <?php $staff_id = @$_GET['id']; ?>
	<form action="staff-profile?id=<?php echo $staff_id ?>" method="post" role="form">
